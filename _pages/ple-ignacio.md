...
layout: page
title: PLE
permalink: /ple-ignacio/
nav: true
nav_order: 7
---

<!-- Contenedor principal del Dashboard del PLE -->
<div class="ple-dashboard">
  
  <!-- Barra de búsqueda superior -->
  <div class="search-container">
    <div class="search-wrapper">
      <i class="fa-solid fa-magnifying-glass search-icon"></i>
      <input type="text" id="search-input" class="search-input" placeholder="Buscar en el PLE, herramientas o enlaces..." aria-label="Buscar en el PLE">
      <button id="clear-search" class="clear-search-btn" style="display: none;" title="Limpiar búsqueda">
  ...
  <div class="dashboard-layout">
    ...
    <main class="main-content">
      <h2 id="active-category-title" class="active-title">Cargando PLE...</h2>

      <!-- Contenido de la página (HTML limpio) -->
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch('{{ "/assets/json/ple-ignacio.json" | relative_url }}')
      .then(response => {
        if (!response.ok) {
      ...
      .catch(error => {
        ...
        activeCategoryTitle.textContent = "Error al cargar el PLE";
        categoryContentContainer.innerHTML = "<p>Lo sentimos, ha ocurrido un problema al acceder a las páginas del PLE. Por favor, inténtelo de nuevo más tarde.</p>";
      });

    // 2. Renderizar categorías en la barra lateral
    function renderSidebarCategories() {
      categoriesList.innerHTML = "";
      pleData.forEach(item => {
    ...
    function selectCategory(categoryId) {
      ...
      const activeItem = pleData.find(item => item.id === categoryId);
    ...
    function handleRouting() {
      ...
      const categoryIds = pleData.map(item => item.id);
      
      if (hash && categoryIds.includes(hash)) {
        selectCategory(hash);
      } else {
        // Por defecto mostramos la primera categoría disponible (Inicio)
    ...
    function performSearch(query) {
      ...
      activeCategoryTitle.textContent = "Resultados de la búsqueda";
      ...
      pleData.forEach(category => {
        // Buscamos sobre el texto plano, evitando etiquetas HTML
        const tempEl = document.createElement("div");
        tempEl.innerHTML = category.content;
        
        // Buscamos coincidencias
        if (tempEl.textContent.toLowerCase().includes(query) || category.title.toLowerCase().includes(query)) {
          matchedCards.push({
      ...
      if (matchedCards.length === 1) {
        searchStatus.textContent = "Se ha encontrado 1 página coincidente.";
      } else {
        searchStatus.textContent = `Se han encontrado ${matchedCards.length} páginas coincidentes.`;
      }
    ...
    function renderSearchResults(results) {
      ...
      if (results.length === 0) {
        cardsGrid.innerHTML = `<div style="grid-column: 1 / -1; text-align: center; padding: 40px; color: var(--dashboard-text-muted);">
          ...
          No se han encontrado páginas que coincidan con la búsqueda.
        </div>`;
        return;
      }
      ...
      results.forEach(result => {
        ...
        catLink.textContent = result.categoryTitle;
        catLink.style.color = "inherit";
        catLink.style.textDecoration = "none";
        
        catHeader.appendChild(catLink);
        
        // Cuerpo de la tarjeta con el contenido original
        const body = document.createElement("div");
        body.className = "resource-card-body";
        body.innerHTML = result.html;

        // Aseguramos que los enlaces internos de las tarjetas también cambien de hash, y los externos se abran en nueva pestaña
